...
SUPPORTED_EXTENSIONS = {".pdf", ".docx", ".doc", ".xlsx", ".xls", ".eml", ".emlx", ".msg", ".mbox", ".db"}
...
def extract_zip(zip_path, output_dir=None):
    ...
    type_dirs = {
        ...
        ".emlx":  os.path.join(output_dir, "emails"),
        ".msg":   os.path.join(output_dir, "emails"),
        ".mbox":  os.path.join(output_dir, "emails"),
        ".db":    os.path.join(output_dir, "databases"),
    }
...
def index_zip(zip_path, collection, verbose=True, acl=None):
    """
    ...
    acl — optional dict {"uploaded_by": str, "dept_ids": [int], "is_public": bool}.
          When present, EACH FILE INSIDE THE ZIP is registered separately in
          auth.db with its own content-hash doc_id. That matters: a ZIP is a
          transport container, not a document. Tagging the ZIP as "HR only"
          while its 40 inner files carry no permissions of their own would mean
          those files are individually unreachable — or, worse, reachable by
          everyone if the ACL were keyed on the ZIP.
          When acl is None the files are indexed with a doc_id but left
          unregistered, so only admins can see them until someone tags them.
    """
    import config
    ...
    for ext, file_paths in result["extracted"].items():
        ...
        for fpath in file_paths:
            ...
            try:
                ...
                # Copy into permanent library BEFORE the temp folder is
                # deleted below, so file_path stays valid after indexing.
                permanent_path = store_in_library(fpath, original_name=fname, origin_tag=f"zip:{zip_name}")

                # Content hash of the INNER file, computed before the temp
                # folder is removed. This is the ACL key.
                did = compute_doc_id(fpath)
