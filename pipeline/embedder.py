...
def embed_text(text):
    ...
    Cached (lru_cache, 256 entries) because this function is only called
    for QUERIES, never for bulk chunk indexing — indexer.py embeds chunks
    directly via model.encode() in batches, bypassing this function
    entirely. So the cache exists purely to make repeat/near-repeat user
    queries (a demo re-asking the same question, a user re-running a
    search after tweaking filters) skip the encode() call, without ever
    growing unbounded from one-off chunk text during indexing.

    Safe to share the cached return value across callers: the result is
    only ever read (passed to collection.query(...) or a cosine-sim calc),
